...
CREATE TABLE IF NOT EXISTS travel_dwh.analytics.daily_revenue_by_type (
  business_date DATE,
  booking_type STRING,
  total_amount DOUBLE,
...
DELETE FROM travel_dwh.analytics.daily_revenue_by_type
...
INSERT INTO travel_dwh.analytics.daily_revenue_by_type (business_date, booking_type, total_amount, total_quantity)
SELECT
...
FROM travel_dwh.bronze.booking_inc
WHERE business_date = COALESCE(TRY_CAST(:arrival_date AS DATE), current_date())
